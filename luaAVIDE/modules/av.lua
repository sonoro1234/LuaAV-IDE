...
local argx ={...}
for i=0,2 do print("arg"..i..":"..tostring(argx[i]).."\n") end
local argc = select("#", ...)
local modulename = ...
...
local script_filename = not is_module and arg[is_module and 0 or 1]
...
if ffi.os == "Windows" then
	...
	local slepperlinda = lanes.linda()
	av.sleepl = function(s)
		lanes.timer(slepperlinda,"wait",s,0)
		slepperlinda:receive("wait")
	end
...
local function run()
	t0run = av.time()
	while av.step() do 
		local has_to_sleep= math.max(period - (av.time() - t0run),0)
		if has_to_sleep > 0.01 then
			av.sleep(has_to_sleep)
		end
		local t1 = av.time()
		local last = t1 - t0run
		if last > period*1.1 then
			print("last",last,"has_to_sleep",has_to_sleep)
		end
		t0run = t1
	end
	print("done")
...
local function runLanes()
	t0run = av.time()
	local glinda = lanes.linda()
	lanes.timer(glinda,"wait",0,period)
	while av.step() do 
		local key,timenow = glinda:receive("wait")
		---[[
		local t1 = timenow
		local last = t1 - t0run
		if last > period*1.1 then
